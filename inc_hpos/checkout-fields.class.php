    ...
    */
    ...
    function save_custom_field_on_order_meta( $order_id ) {
        ...
        if ( ! empty( $_POST['billing_kennitala'] ) ) {
            $order->update_meta_data( '_billing_kennitala', sanitize_text_field( $_POST['billing_kennitala'] ) );
        }
        
        if ( ! empty( $_POST['postis_dpo_postbox'] ) ) {
            $order->update_meta_data( 'postis_dpo_postbox', sanitize_text_field( $_POST['postis_dpo_postbox'] ) );
        }
        
        if ( ! empty( $_POST['postis_dpo_phonenumber'] ) ) {
            $order->update_meta_data( 'postis_dpo_phonenumber', sanitize_text_field( $_POST['postis_dpo_phonenumber'] ) );
        }
        
        if ( ! empty( $_POST['postis_dpt_parcelpoints'] ) ) {
            $order->update_meta_data( 'postis_dpt_parcelpoints', sanitize_text_field( $_POST['postis_dpt_parcelpoints'] ) );
        }
        
        if ( ! empty( $_POST['postis_dpt_phonenumber'] ) ) {
            $order->update_meta_data( 'postis_dpt_phonenumber', sanitize_text_field( $_POST['postis_dpt_phonenumber'] ) );
        }
        ...
        if ( isset($_POST['shipping_method']) && !empty( $_POST['shipping_method'] ) ) {
            foreach ($_POST['shipping_method'] as $shippment_index => $shipping_service) {
                $order->update_meta_data( 'postis_shipping_method', sanitize_text_field( $shipping_service ) );
            }
        }
        $order->save();
    }
